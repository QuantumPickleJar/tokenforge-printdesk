-- TokenForge PrintDesk v0.1 core schema
-- Run with: supabase db push
-- If you see: relation "supabase_migrations.schema_migrations" does not exist,
-- verify `supabase link --project-ref <ref>` and repair/init migration metadata with Supabase CLI
-- before making dashboard-only production table changes.

create extension if not exists pgcrypto;

create type public.request_type as enum ('public_quote', 'family_free', 'owner_internal');
create type public.request_status as enum (
  'submitted', 'reviewing', 'needs_more_info', 'accepted', 'accepted_for_quote',
  'quote_draft', 'quoted', 'quote_viewed', 'quote_accepted', 'payment_pending',
  'paid', 'printing', 'ready_for_pickup', 'shipped', 'completed', 'declined',
  'canceled', 'archived'
);
create type public.payment_status as enum ('not_required', 'not_started', 'pending', 'paid', 'waived', 'failed', 'refunded');
create type public.processing_job_status as enum ('pending', 'claimed', 'processing', 'completed', 'failed');
...
create table public.family_members (
  ...
  verification_todo text default 'Future: require magic-link verification before trusting email-only classification.',
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  unique (lower(email))
);
...
create table public.print_requests (
  ...
  request_type public.request_type not null default 'public_quote',
...
returns boolean language sql stable security definer set search_path = public as $$
  select exists (
    select 1 from public.owner_members om
...
create or replace function public.request_exists_for_upload(p_request_id uuid)
returns boolean language sql stable security definer set search_path = public as $$
  select exists (select 1 from public.print_requests pr where pr.id = p_request_id and pr.deleted_at is null);
$$;

create or replace function public.submit_print_request(request_payload jsonb)
...
declare
  ...
  v_member_id uuid;
  v_group_id uuid;
  v_request_type public.request_type := 'public_quote';
  v_payment_required boolean := true;
  v_payment_status public.payment_status := 'not_started';
begin
  if length(trim(coalesce(request_payload->>'requester_name', ''))) = 0 then
    raise exception 'requester_name is required';
  end if;
  if length(v_email) = 0 or position('@' in v_email) = 0 then
    raise exception 'valid requester_email is required';
  end if;
  if length(trim(coalesce(request_payload->>'request_title', ''))) = 0 then
    raise exception 'request_title is required';
  end if;
  if length(trim(coalesce(request_payload->>'request_description', ''))) = 0 then
    raise exception 'request_description is required';
  end if;
  if coalesce((request_payload->>'licensing_confirmed')::boolean, false) is not true then
    raise exception 'licensing confirmation is required';
  end if;

  select fm.id, fm.group_id into v_member_id, v_group_id
  from public.family_members fm
  left join public.family_groups fg on fg.id = fm.group_id
  where lower(fm.email) = v_email
    and fm.active = true
    and coalesce(fg.active, true) = true
  limit 1;

  if v_member_id is not null then
    v_request_type := 'family_free';
  ...
  insert into public.print_requests (
    ...
    request_type, status, payment_required, payment_status, reply_requested,
    licensing_confirmed, personal_design, model_source_url, shipping_requested,
    shipping_notes, material_color_id, material_request_notes, advanced_mode_used,
    layer_height, infill_type, infill_percent, wall_count,
    ...
    v_id,
    trim(request_payload->>'requester_name'),
    v_email,
    trim(request_payload->>'request_title'),
    trim(request_payload->>'request_description'),
    v_request_type,
    'submitted',
    v_payment_required,
    v_payment_status,
    coalesce((request_payload->>'reply_requested')::boolean, false),
    true,
    coalesce((request_payload->>'personal_design')::boolean, false),
    nullif(trim(coalesce(request_payload->>'model_source_url', '')), ''),
    coalesce((request_payload->>'shipping_requested')::boolean, false),
    nullif(trim(coalesce(request_payload->>'shipping_notes', '')), ''),
    nullif(request_payload->>'material_color_id', '')::uuid,
    nullif(trim(coalesce(request_payload->>'material_request_notes', '')), ''),
    coalesce((request_payload->>'advanced_mode_used')::boolean, false),
    nullif(request_payload->>'layer_height', '')::numeric,
    nullif(request_payload->>'infill_type', ''),
    nullif(request_payload->>'infill_percent', '')::integer,
    nullif(request_payload->>'wall_count', '')::integer,
    nullif(request_payload->>'rough_estimated_volume_cm3', '')::numeric,
    nullif(request_payload->>'rough_estimated_grams', '')::numeric,
    nullif(request_payload->>'rough_estimated_material_cost', '')::numeric,
    nullif(request_payload->>'rough_estimate_version', ''),
  ...
  insert into public.request_events (request_id, event_type, message, actor)
  values (v_id, 'submitted', 'Request submitted by public request form.', 'requester');

  insert into public.submission_events (request_id, requester_email, event_type)
  values (v_id, v_email, 'request_submitted');

  insert into public.notification_logs (request_id, notification_type, recipient_role, status, payload)
  values (
    v_id,
    case when v_member_id is null then 'owner_new_public_quote_request' else 'owner_new_family_request' end,
    'owner',
    'queued',
    jsonb_build_object('request_id', v_id, 'request_type', v_request_type)
  );
...
create or replace function public.record_request_file(
  p_request_id uuid,
  p_storage_path text,
...
declare
  v_file_id uuid;
begin
  if p_size_bytes <= 0 or p_size_bytes > 41943040 then
    raise exception 'STL must be non-empty and 40 MB or less';
  end if;
  if lower(p_original_filename) not like '%.stl' or lower(p_storage_path) not like '%.stl' then
    raise exception 'Only STL files are accepted';
  end if;
  if p_storage_path <> format('requests/%s/model.stl', p_request_id) then
    raise exception 'Invalid request model storage path';
  end if;
  ...
  insert into public.request_events (request_id, event_type, message, actor)
  values (p_request_id, 'file_uploaded', 'Requester uploaded STL model metadata.', 'requester');
...
create or replace function public.get_public_quote_by_token(p_quote_token text)
...
begin
  ...
  join public.print_requests pr on pr.id = q.request_id
  where q.quote_token = p_quote_token
...
create or replace function public.respond_to_quote(p_quote_token text, p_response text)
...
begin
  ...
  if p_response = 'accepted' then
    ...
    update public.print_requests
      set status = case when v_quote.payment_url is null then 'quote_accepted' else 'payment_pending' end,
          payment_status = case when v_quote.payment_url is null then payment_status else 'pending' end
      where id = v_quote.request_id;
    insert into public.notification_logs (request_id, quote_id, notification_type, recipient_role, status)
    values (v_quote.request_id, v_quote.id, 'owner_quote_accepted', 'owner', 'queued');
    return true;
    ...
    insert into public.notification_logs (request_id, quote_id, notification_type, recipient_role, status)
    values (v_quote.request_id, v_quote.id, 'owner_quote_declined', 'owner', 'queued');
    return true;
  end if;
...
create policy "Owners can read owner members" on public.owner_members for select to authenticated using (public.is_owner_admin());
create policy "Owners can manage owner members" on public.owner_members for all to authenticated using (public.is_owner_admin()) with check (public.is_owner_admin());

create policy "Public can read active materials" on public.materials for select to anon, authenticated using (active = true);
...
create policy "Public can read active material colors" on public.material_colors for select to anon, authenticated using (active = true and exists (select 1 from public.materials m where m.id = material_id and m.active = true));
...
create policy "Public reads published gallery items" on public.gallery_items for select to anon, authenticated using (published = true and deleted_at is null);
...
create policy "Public reads published gallery images" on public.gallery_images for select to anon, authenticated using (exists (select 1 from public.gallery_items gi where gi.id = gallery_item_id and gi.published = true and gi.deleted_at is null));
...
create policy "Owners manage notifications" on public.notification_logs for all to authenticated using (public.is_owner_admin()) with check (public.is_owner_admin());
...
create policy "Public uploads controlled request STL paths" on storage.objects for insert to anon, authenticated
with check (
  bucket_id = 'request-models'
  and storage.filename(name) = 'model.stl'
  and lower(storage.extension(name)) = 'stl'
...
using (bucket_id = 'request-models' and public.is_owner_admin())
with check (bucket_id = 'request-models' and public.is_owner_admin());

create policy "Public reads gallery image objects" on storage.objects for select to anon, authenticated
...
insert into public.materials (id, name, material_type, density_g_cm3, cost_per_kg, active, print_notes, sort_order)
values
...
insert into public.material_colors (id, material_id, color_name, brand, hex_color, active, sort_order)
values
